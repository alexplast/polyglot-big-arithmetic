...
def benchmark():
    ...
    print(f"--- Running BigInt Benchmark ---")
    ...
    # Sort primarily by Factorial time (most balanced test)
    # Treat None as infinity for sorting
    results.sort(key=lambda x: x['fact'] if x['fact'] is not None else 999999999)

    # Determine baseline (fastest factorial)
    ...
    for r in results:
        ...
        # Calculate Relative Speed based on Factorial
        rel_str = "—"
        if r['fact'] is not None:
            if r['fact'] == baseline_fact:
                rel_str = "1.00x"
            else:
                rel_str = f"{r['fact'] / baseline_fact:.2f}x"
